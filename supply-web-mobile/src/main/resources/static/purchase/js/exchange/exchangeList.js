...
function selectProductBySupplierId() {
    $.ajax({
        type: "post",
        url: "/purchase/exchangeList",
        data: {
            "pageNo": pageNo,
            "pageSize": pageSize
        },
...
$('.all_choose').on('click', function () {
    ...
    if (dom.hasClass('iconyuan')) {
        dom.addClass('iconqueren').removeClass('iconyuan')
        $('.cart_list_item').each(function () {
            $(this).find('.iconyuan').addClass('iconqueren').removeClass('iconyuan')
        })
    } else {
        dom.addClass('iconyuan').removeClass('iconqueren')
        $('.cart_list_item').each(function () {
            $(this).find('.iconqueren').removeClass('iconqueren').addClass('iconyuan')
        })
    }
})
